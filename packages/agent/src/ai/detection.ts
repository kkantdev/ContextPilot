 ...
 */
export function normalizeModelTag(tag: unknown): string {
 ...
 */
export function isExactOrVariant(tag: string, exactModel: string): boolean {
  const normalized = normalizeModelTag(tag);
  if (normalized === exactModel) return true;
  if (normalized.startsWith(`${exactModel}-`)) return true;
  return false;
}
 ...
 */
export async function detectOllama(endpoint: string, configuredModel: string): Promise<OllamaDetectionResult> {
  let tags: string[] = [];
  try {
